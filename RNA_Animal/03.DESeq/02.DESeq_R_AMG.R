...
coldata_df_R_AMG_IPSI_O_CR <- coldata_df_R_D %>% filter(TYPE_1 == "R_AMG_IPSI_Y_CR")
filtered_samples_R_AMG_IPSI_O_CR <- rownames(coldata_df_R_AMG_IPSI_O_CR)
adjusted_df_R_AMG_IPSI_O_CR <- adjusted_df_R_D[, colnames(adjusted_df_R_D) %in% filtered_samples_R_AMG_IPSI_O_CR]
dds_data <- adjusted_df_R_AMG_IPSI_O_CR
dds_coldata <- coldata_df_R_AMG_IPSI_O_CR
Target_comparison <- 'R_AMG_IPSI_Y_CR'


################################################
contrast_vector <- c("Treatment_Lateral","AMG_STI_IPSI","CTL_AMG_STI_IPSI")
