...
coldata_df_R_TBI_IPSI_O_CR <- coldata_df_R_C %>% filter(TYPE_1 == "R_TBI_IPSI_Y_CR")
filtered_samples_R_TBI_IPSI_O_CR <- rownames(coldata_df_R_TBI_IPSI_O_CR)
adjusted_df_R_TBI_IPSI_O_CR <- adjusted_df_R_C[, colnames(adjusted_df_R_C) %in% filtered_samples_R_TBI_IPSI_O_CR]
dds_data <- adjusted_df_R_TBI_IPSI_O_CR
dds_coldata <- coldata_df_R_TBI_IPSI_O_CR
Target_comparison <- 'R_TBI_IPSI_Y_CR'


################################################
contrast_vector <- c("Treatment_Lateral","TBI_IPSI","CTL_TBI_IPSI")
