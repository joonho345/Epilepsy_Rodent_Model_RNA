...
Target <- 'FILTERED_1_MTLEALL_NL'
Target_name <- 'MTLEALL'
#Target <- 'FILTERED_2_MTLEHS_NL'
#Target_name <- 'MTLEHS'
#Target <- 'FILTERED_3_MTLE_NL'
#Target_name <- 'MTLE'
...
correlation_matrix_W <- correlation_matrix_W[Target, ]
pvalue_matrix_W <- pvalue_matrix_W[Target, ]

treatment_color_vector <- c("KAI_ST" = "#fc8d62", "KAI_IP" = "#e5c494", "PILO_IP" = "#66c2a5",
...
# Loop through each phase and generate plots
for (phase in names(phase_list)) {
  
  plot_data <- phase_list[[phase]]
  
  plot_target <- ggplot(plot_data, aes(x = Group, y = Correlation)) +
...
for (phase in names(phase_list)) {
  ...
  file_name <- paste0("/home/joonho345/3_RNA/RNA_Human/14.DEG_shared_count/01.Dotplot_correlation_", phase, ".png")
