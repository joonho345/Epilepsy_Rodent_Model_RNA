...
plot_data <- transformed_df %>%
  pivot_longer(
    cols = c(Gene_Count_up, Gene_Count_down, Correlation),
    names_to = "Metric",
    values_to = "Value"
  )
plot_data <- plot_data %>%
  mutate(Metric = factor(Metric, levels = c("Correlation", "Gene_Count_up", "Gene_Count_down")))
scale_factor <- 400 / 0.4  # Ratio of primary y-axis range to secondary y-axis range
...
plot_target <- ggplot(plot_data, aes(x = Group)) +
  ...
  geom_line(data = filter(plot_data, Metric %in% c("Gene_Count_up", "Gene_Count_down")),
            aes(y = Value / scale_factor, group = Metric, color = Metric), size = 1, alpha = 0.8) +
  geom_point(data = filter(plot_data, Metric %in% c("Gene_Count_up", "Gene_Count_down")),
             aes(y = Value / scale_factor, color = Metric), size = 3, alpha = 0.8) +
  geom_line(data = filter(plot_data, Metric == "Correlation"), size = 1, alpha = 0.8,
            aes(y = Value, group = Metric, color = Metric)) +
  geom_point(data = filter(plot_data, Metric == "Correlation"), size = 3, alpha = 0.8,
             aes(y = Value, color = Metric)) +
  
  scale_y_continuous(
    name = "Cross-Species Correlation",
    limits = c(-0.025, 0.375),  # Set range for primary y-axis
    sec.axis = sec_axis(~ . * scale_factor, name = "Shared Gene Counts")
  ) +
  scale_color_manual(values = c("Gene_Count_up" = "#313695", "Gene_Count_down" = "#1a9850", "Correlation" = "#d73027")) +
  labs(color = "Metric") +
...
file_name <- "/home/joonho345/3_RNA/RNA_Human/14.DEG_shared_count/01.Dotplot_shared_counts.png"
...
file_name <- "/home/joonho345/3_RNA/RNA_Human/14.DEG_shared_count/01.Dotplot_shared_counts_without.png"
ggsave(file_name, plot = plot_target, width = plot_width, height = plot_height, dpi = plot_dpi, units = plot_unit)
...
plot_target <- ggplot(plot_data, aes(x = Group)) +
  ...
  geom_line(data = filter(plot_data, Metric %in% c("Gene_Count_up", "Gene_Count_down")),
            aes(y = Value / scale_factor, group = Metric, color = Metric), size = 1, alpha = 0.8) +
  geom_point(data = filter(plot_data, Metric %in% c("Gene_Count_up", "Gene_Count_down")),
             aes(y = Value / scale_factor, color = Metric), size = 4, alpha = 0.8) +
  geom_line(data = filter(plot_data, Metric == "Correlation"), size = 1, alpha = 0.8,
            aes(y = Value, group = Metric, color = Metric)) +
  geom_point(data = filter(plot_data, Metric == "Correlation"), size = 4, alpha = 0.8,
             aes(y = Value, color = Metric)) +
...
file_name <- "/home/joonho345/3_RNA/RNA_Human/14.DEG_shared_count/01.Dotplot_shared_counts_without_tick.png"
ggsave(file_name, plot = plot_target, width = plot_width, height = plot_height, dpi = plot_dpi, units = plot_unit)
...
file_name <- "/home/joonho345/3_RNA/RNA_Human/14.DEG_shared_count/01.Dotplot_shared_counts_without_tick_line.png"
ggsave(file_name, plot = plot_target, width = plot_width, height = plot_height, dpi = plot_dpi, units = plot_unit)
