...
adjusted_TPM_id_df <- read.table("/home/joonho345/3_RNA/RNA_Human/07.Enrichment/GSEA_01.Matrix_GTF/adjusted_merged_matrix_1_id_TPM.txt", 
                                 sep = "\t", header = TRUE, row.names = 1)
#### import coldata ####
coldata_df <- read.table("/home/joonho345/3_RNA/RNA_Human/02.Quantification/filtered_coldata.txt", sep = "\t", header = TRUE, row.names = 1, fill = TRUE)
...
# Grouping FCD, TSC as OtherEpilepsy and AUD, BPD, MDD, SCZD as Psychiatric
coldata_df_GSEA <- coldata_df %>%
  mutate(Diagnosis_GSEA = case_when(
    Diagnosis %in% c("FCD", "TSC") ~ "OtherEpilepsy",
    Diagnosis %in% c("AUD", "BPD", "MDD", "SCZD") ~ "Psychiatric",
    TRUE ~ Diagnosis  # Keep other diagnoses unchanged
  ))

# Extract the filtered sample names for GSEA
filtered_samples_GSEA <- rownames(coldata_df_GSEA)
adjusted_df_GSEA <- adjusted_TPM_id_df[, colnames(adjusted_TPM_id_df) %in% filtered_samples_GSEA]
# Ensure that `coldata_df_GSEA` and `adjusted_df_GSEA` have matched sample names
adjusted_df_GSEA <- adjusted_df_GSEA[, filtered_samples_GSEA]
...
gsea_output_path <- paste0("/home/joonho345/3_RNA/RNA_Human/07.Enrichment/GSEA_02.Matrix_GSEA/gsea_formatted_matrix_",group_name,".txt")
...
output_file_path <- paste0("/home/joonho345/3_RNA/RNA_Human/07.Enrichment/GSEA_02.Matrix_GSEA/phenotype_matrix_",group_name,".txt")
