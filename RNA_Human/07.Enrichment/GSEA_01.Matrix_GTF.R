...
# Create transcript_id column
#extract_transcript_id <- function(v9_string) {
#  # Use a regular expression to find and extract the gene_id value
#  matches <- regmatches(v9_string, regexpr('transcript_id "\\S+"', v9_string))
#  transcript_id <- ifelse(length(matches) > 0, sub('transcript_id "(\\S+)"', '\\1', matches), NA)
#  return(transcript_id)
#}
#transcript_ids <- sapply(gtf_table$V9, extract_transcript_id)
...
#### Expression matrix ####
adjusted_TPM_df <- read.table("/home/joonho345/3_RNA/RNA_Human/04.Normalization/adjusted_merged_matrix_1_TPM.txt", 
...
write.table(adjusted_TPM_id_matrix, file = "/home/joonho345/3_RNA/RNA_Human/07.Enrichment/GSEA_01.Matrix_GTF/adjusted_merged_matrix_1_id_TPM.txt", 
