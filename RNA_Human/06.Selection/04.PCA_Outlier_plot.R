...
#### Read outlier files ####
outlier_dir <- "/home/joonho345/1_Epilepsy_RNA/RNA_Human/06.Selection/03.PCA_Outliers"

# Try to read the combined all outliers file first (most efficient)
all_outliers_file <- paste0(outlier_dir, "/03.ALL_Outliers_PCA.txt")
if (file.exists(all_outliers_file) && file.info(all_outliers_file)$size > 0) {
  ...
  mtle_file <- paste0(outlier_dir, "/03.MTLE_Outliers_PCA.txt")
  nl_file <- paste0(outlier_dir, "/03.NL_Outliers_PCA.txt")
...
mtle_outliers <- if (file.exists(paste0(outlier_dir, "/03.MTLE_Outliers_PCA.txt")) && 
                     file.info(paste0(outlier_dir, "/03.MTLE_Outliers_PCA.txt"))$size > 0) {
  tryCatch({
    tmp <- trimws(read.table(paste0(outlier_dir, "/03.MTLE_Outliers_PCA.txt"), stringsAsFactors = FALSE)$V1)
    tmp[tmp != ""]
  }, error = function(e) {
    character(0)
  })
...
nl_outliers <- if (file.exists(paste0(outlier_dir, "/03.NL_Outliers_PCA.txt")) && 
                   file.info(paste0(outlier_dir, "/03.NL_Outliers_PCA.txt"))$size > 0) {
  tryCatch({
    tmp <- trimws(read.table(paste0(outlier_dir, "/03.NL_Outliers_PCA.txt"), stringsAsFactors = FALSE)$V1)
    tmp[tmp != ""]
  }, error = function(e) {
    character(0)
  })
...
output_dir <- "/home/joonho345/1_Epilepsy_RNA/RNA_Human/06.Selection/04.PCA_Outlier_plot"
...
if ("Diagnosis_Sub" %in% colnames(pca_df)) {
  ...
  pca_plot_diagnosis <- pca_plot_diagnosis +
    ...
    labs(x = paste0("PC1 (", variance_explained_pct[1], "%)"),
         ...
         #title = paste0("PCA - TPM Normalized (adjusted_merged_matrix_1_TPM) - TLE vs NL (HIPPO, H_gene_set_W) with PCA-based Outliers")) +
    theme_minimal() +
    theme(
  ...
  file_name_diagnosis <- paste0(output_dir, "/PCA_TPM_TLE_NL_HIPPO_H_gene_set_W_with_PCA_Outliers_Diagnosis.png")
...
for (Target_gene in names(target_genes_config)) {
  ...
  # Save plot as PNG
  file_name_png <- paste0(output_dir, "/Boxplot_", Target_gene, "_with_PCA_Outliers.png")
