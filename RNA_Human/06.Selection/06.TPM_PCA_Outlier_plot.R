...
#### Read outlier files ####
outlier_dir <- "/home/joonho345/1_Epilepsy_RNA/RNA_Human/06.Selection/05.TPM_PCA_Outliers"

# Try to read the combined all outliers file first (most efficient)
all_outliers_file <- paste0(outlier_dir, "/05.ALL_Outliers_TPM_PCA_Common.txt")
if (file.exists(all_outliers_file) && file.info(all_outliers_file)$size > 0) {
  ...
  mtle_file <- paste0(outlier_dir, "/05.MTLE_Outliers_TPM_PCA_Common.txt")
  nl_file <- paste0(outlier_dir, "/05.NL_Outliers_TPM_PCA_Common.txt")
  
  mtle_outliers <- if (file.exists(mtle_file) && file.info(mtle_file)$size > 0) {
    tryCatch({
      tmp <- trimws(read.table(mtle_file, stringsAsFactors = FALSE)$V1)
      tmp[tmp != ""]
    }, error = function(e) {
      character(0)
    })
  ...
  nl_outliers <- if (file.exists(nl_file) && file.info(nl_file)$size > 0) {
    tryCatch({
      tmp <- trimws(read.table(nl_file, stringsAsFactors = FALSE)$V1)
      tmp[tmp != ""]
    }, error = function(e) {
      character(0)
    })
  ...
  # Combine all outliers (union of MTLE and NL)
  ALL_outliers <- unique(c(mtle_outliers, nl_outliers))
}

# Read individual MTLE and NL outliers files (for reporting)
mtle_outliers <- if (file.exists(paste0(outlier_dir, "/05.MTLE_Outliers_TPM_PCA_Common.txt")) && 
                     file.info(paste0(outlier_dir, "/05.MTLE_Outliers_TPM_PCA_Common.txt"))$size > 0) {
  tryCatch({
    tmp <- trimws(read.table(paste0(outlier_dir, "/05.MTLE_Outliers_TPM_PCA_Common.txt"), stringsAsFactors = FALSE)$V1)
    tmp[tmp != ""]
  }, error = function(e) {
    character(0)
  })
...
nl_outliers <- if (file.exists(paste0(outlier_dir, "/05.NL_Outliers_TPM_PCA_Common.txt")) && 
                   file.info(paste0(outlier_dir, "/05.NL_Outliers_TPM_PCA_Common.txt"))$size > 0) {
  tryCatch({
    tmp <- trimws(read.table(paste0(outlier_dir, "/05.NL_Outliers_TPM_PCA_Common.txt"), stringsAsFactors = FALSE)$V1)
    tmp[tmp != ""]
  }, error = function(e) {
    character(0)
  })
...
output_dir <- "/home/joonho345/1_Epilepsy_RNA/RNA_Human/06.Selection/06.TPM_PCA_Outlier_plot"
...
if ("Diagnosis_Sub" %in% colnames(pca_df)) {
  ...
  pca_plot_diagnosis <- pca_plot_diagnosis +
    ...
    #title = paste0("PCA - TPM Normalized (adjusted_merged_matrix_1_TPM) - TLE vs NL (HIPPO, H_gene_set_W) with TPM-PCA Common Outliers")) +
    theme_minimal() +
    theme(
  ...
  file_name_diagnosis <- paste0(output_dir, "/PCA_TPM_TLE_NL_HIPPO_H_gene_set_W_with_TPM_PCA_Common_Outliers_Diagnosis.png")
...
for (Target_gene in names(target_genes_config)) {
  ...
  # Save plot as PNG
  file_name_png <- paste0(output_dir, "/Boxplot_", Target_gene, "_with_TPM_PCA_Common_Outliers.png")
